<script lang="ts">
	...
	const getIconColor = (
		...
		_theme: StoreType<"theme">
	): string => {
		if (_theme === icon.value) return "currentColor";
		if (icon.value === "dark") return "royalblue";
		if (icon.value === "light") return "orange";
		return "currentColor";
	};
</script>
...
<RadioGroup
	...
	items={icons}
	let:item={icon}
	on:change={(e) => ($theme = e.detail)}
>
	<Radio value={icon.value}>
		<Icon title={icon.label} color={getIconColor(icon, $theme)}>
			{#if icon.value === "auto"}
				<path
